  ...
  methods: {
    ...
    beforeUpload(e) {
      ...
      Object.keys(e.file).map(item => {
        this.fileform.fileIdList.push(item);
      })
      let fd = new FormData()
      fd.append('key', e.file)
      console.log("fd",fd)
      axios.post('http://192.168.1.133:10081/ykym/gateWay/uploadFile', fd, {hearders:{"Content-Type": "multipart/form-data"}}).then(response => {
