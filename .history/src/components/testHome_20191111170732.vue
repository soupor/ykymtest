    ...
    <div v-else-if="form.bizCode == 'invoiceApply'">
        <el-form ref="form" :model="applyform" label-width="100px">
          ...
          <el-form-item v-if="applyform.type == '0'" label="是否电子发票" class="item">
            <el-radio v-model="applyform.needPolicyFile" label="1">是</el-radio>
            <el-radio v-model="applyform.needPolicyFile" label="0">否</el-radio>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    beforeUpload(e) {
      console.log("file",e)
      this.fileform.fileIdList.push(e.file);
      let fd = new FormData()
      fd.append('key', e.file)
      axios.post("http://192.168.1.133:10081/ykym/gateWay/uploadFile", fd, {headers:'Content-Type: multipart/form-data'}).then(response => {
