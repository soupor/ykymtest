  ...
  <div class="testHome">
    ...
    <div v-else-if="form.bizCode == 'invoiceApply'">
        <el-form ref="form" :model="applyform" label-width="100px">
          <el-form-item label="合作方订单ID" class="item" >
            <el-input v-model="applyform.outOrderId"></el-input>
          </el-form-item>
          <el-form-item label="发票类型" class="item" >
            <el-select v-model="applyform.type" placeholder="请选择文件类型">
              <el-option label="普票" value="0" ></el-option>
              <el-option label="专票" value="1" ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item v-if="applyform.type == '0'" label="是否电子发票" class="item">
            <el-radio v-model="applyform.isElectronic" label="1">是</el-radio>
            <el-radio v-model="applyform.isElectronic" label="0">否</el-radio>
          </el-form-item>
          <el-form-item v-else label="是否电子发票" class="item">
            <p>{{ applyform.isElectronic = "0" }}</p>
          </el-form-item>
          <el-form-item label="公司名称" class="item" >
            <el-input v-model="applyform.corporateName"></el-input>
          </el-form-item>
          <el-form-item label="注册地址" class="item" >
            <el-input v-model="applyform.registeredAddress"></el-input>
          </el-form-item>
          <el-form-item label="开户银行名称" class="item" >
            <el-input v-model="applyform.bankName"></el-input>
          </el-form-item>
          <el-form-item label="开户银行账号" class="item" >
            <el-input v-model="applyform.bankAccount"></el-input>
          </el-form-item>
          <el-form-item label="开票电话" class="item" >
            <el-input v-model="applyform.invoiceMobile"></el-input>
          </el-form-item>
          <el-form-item label="统一社会信用代码或纳税人识别号" class="item" >
            <el-input v-model="applyform.creditOrTaxCode"></el-input>
          </el-form-item>
          <el-form-item label="收件人姓名" class="item" >
            <el-input v-model="applyform.contactName"></el-input>
          </el-form-item>
          <el-form-item label="联系电话" class="item" >
            <el-input v-model="applyform.contactNumber"></el-input>
          </el-form-item>
          <el-form-item label="通讯地址" class="item" >
            <el-input v-model="applyform.contactAddress"></el-input>
          </el-form-item>
          <el-form-item label="邮政编码" class="item" >
            <el-input v-model="applyform.zipCode"></el-input>
          </el-form-item>
        </el-form>
    </div>
...
export default {
  ...
  methods: {
    ...
    Upload(e) {
      console.log("file",e)
      // Object.keys(e.file).map(item => {
      //   console.log("item",item);
      //   this.fileform.fileIdList.push(e.file[item]);
      //   console.log("fileIdList",this.fileform.fileIdList);
      // })
      let param = new window.FormData();
      let arg = {
        version: this.form.gwVer,
        sender: this.form.sender,
        senderFileId: this.fileform.fileIdList,
        media: e.file,
        mediaHash: '',
        signType: this.form.signType,
        sign: this.form.sign,
      }
      Object.keys(this.arg).map(item => {
					if(typeof  this.importArg[item] === "string" || typeof  this.importArg[item] === "number") {
						param.append(item, this.arg[item]);
					}
      });
      this.fileform.fileIdList.map(fileItem => {
					param.append("fileName", fileItem)
      });
      // fileform.fileIdList
      let header = {"Content-Type": "multipart/ofrm-data"}
      this.$api.post('http://192.168.1.133:10081/ykym/gateWay/uploadFile', arg, header).then(response => {
    ...
    initDate() {
      this.form.bizData.insStartDate = this.others.insuranceDate[0]
      this.form.bizData.insEndDate = this.others.insuranceDate[1]
      console.log("insStartDate",this.form.bizData.insStartDate)
    ...
    getSign(flag) {
      ...
      axios.post('http://192.168.1.133:10081/ykym/gateWay/getSign', arg).then(response => {
    ...
    submit() {
      axios.post('http://192.168.1.133:10081/ykym/gateWay/zuulTest',this.res.data).then(response => {
