...
<template>
  <div class="testHome">
    ...
    <el-button type="primary" @click="goback()" class="buttonPost" >返回</el-button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    beforeUpload(e) {
      ...
      fd.append('key', e.file)
      console.log("fd",fd)
      let arg = {
        version: this.form.gwVer,
        snder: this.form.sender,
      ...
      this.instance.get('http://192.168.1.133:10081/ykym/gateWay/uploadFile', arg).then(response => {
            console.log("fileres",response);
          }).catch(error => {
            console.log('error',error);
            alert('网络错误，无法访问123')
          })
      // axios.get('http://192.168.1.133:10081/ykym/gateWay/uploadFile', fd, this.headers).then(response => {
      //   console.log("fileres",response);
      // }).catch(error => {
      //   console.log('error',error);
      //   alert('网络错误，无法访问123')
